...
var ShopFrontend;
(ShopFrontend||(ShopFrontend = {}));
(function (ShopFrontend) {
const moduleName = `ShopFrontend`;
const _ = {};


let _n;
let ProduitRAM=class ProduitRAM extends AventusSharp.RAM.RamHttp {
    /**
     * @inheritdoc
     */
    defineRoutes() {
        return new Shop.Routes.ProduitRouter();
    }
    /**
     * Create a singleton to store data
     */
    static getInstance() {
        return Aventus.Instance.get(ProduitRAM);
    }
    /**
     * @inheritdoc
     */
    ...
    getTypeForData(objJson) {
        return Shop.Data.Produit;
    }
}
ProduitRAM.Namespace=`ShopFrontend`;
_.ProduitRAM=ProduitRAM;

const App = class App extends Aventus.WebComponent {
    get 'produits'() {
						return this.__watch["produits"];
					}
					set 'produits'(val) {
						this.__watch["produits"] = val;
					}    __registerWatchesActions() {
    this.__addWatchesActions("produits");    super.__registerWatchesActions();
}
    static __style = ``;
    __getStatic() {
        return App;
    ...
    this.__getStatic().__template.setHTML({
        blocks: { 'default':`<template _id="app_0"></template>` }
    });
}
    __registerTemplateAction() { super.__registerTemplateAction();const templ0 = new Aventus.Template(this);templ0.setTemplate(`    <div _id="app_1"></div>`);templ0.setActions({
  "content": {
    "app_1°@HTML": {
      "fct": (c) => `${c.print(c.comp.__6ecc706ca23684097c39aea921ea7e21method1(c.data.produit))}`,
      "once": true
    }
  }
                    ...
                    anchorId: 'app_0',
                    template: templ0,
                simple:{data: "this.produits",item:"produit"}}); }
    getClassName() {
        return "App";
    }
    __defaultValuesWatch(w) { super.__defaultValuesWatch(w); w["produits"] = []; }
    __upgradeAttributes() { super.__upgradeAttributes(); this.__correctGetter('produits'); }
    async loadData() {
        // let ram = ProduitRAM.getInstance();
        // let result = await ram.getList();
        // this.produits = result;
        this.produits = (await (new Shop.Routes.ProduitRouter()).GetAll()).result;
    }
    postCreation() {
        super.postCreation();
        this.loadData();
    }
    __6ecc706ca23684097c39aea921ea7e21method1(produit) {
        return produit.Nom;
    }
}
App.Namespace=`ShopFrontend`;
App.Tag=`sp-app`;
_.App=App;
if(!window.customElements.get('sp-app')){window.customElements.define('sp-app', App);Aventus.WebComponentInstance.registerDefinition(App);}


for(let key in _) { ShopFrontend[key] = _[key] }
})(ShopFrontend);
